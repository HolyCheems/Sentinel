...
using Sentinel.Models.Geometry;
namespace Sentinel.Models
{
	...
	public class ParkingLot : IID
	{
		...
		public IEnumerable<Annotation> ParkingSpaces {
			get
			{
				return Annotations.Where(a => a.Type == Annotation.AnnotationType.Standard);
			}
		}
				...
				lock (this)
				{
					...
					for (int x = 0; x < difference.Width; x++)
					{
						for (int y = 0; y < difference.Height; y++)
						{
							...
							foreach (Annotation annotation in Annotations.Where(a => a.Contains(pixelPoint) && a.Type != Annotation.AnnotationType.Aisle))
							{
